...
{
    kdDebug() << "Engine is ready" << k_funcinfo << endl;
}
...
{
kdDebug() << "seek" << ms << endl;
    if ( m_pPlayObject )
    {
    kdDebug() << "if ~~~seek" << ms << endl;
        Arts::poTime time;
        time.ms      = ms % 1000;
